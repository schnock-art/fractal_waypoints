...
Absolute source updates are timestamped and coalesced to the latest value once per animation frame. The advanced monitor reports received, applied-frame, coalesced, stale and overflow counts. Disconnect or input change releases both targets and disarms them, but keeps their session relationships; after selecting the input again, inspect and explicitly re-arm them. No browser input identifier is part of the relationship, and no mapping is persisted yet.
