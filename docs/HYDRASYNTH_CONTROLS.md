...
In Perform, choose **Open controller**. The workspace opens on **Live**, which shows only the connected instrument, active layout, current mappings, arm state, and performance-take controls.

To create a relationship, open **Mappings → Add mapping**:

1. Move or generate a control signal and choose **Use this source**. If needed, expand **Browse Hydrasynth controls** or **Enter MIDI address manually**.
2. Choose the currently supported target under its semantic family: Navigation → Zoom or Colour → Palette offset.
3. Tune only the relevant response: Zoom behaviour, or Palette output range/curve/inversion. Choose **Save mapping**, then arm it.

The full 117-entry catalogue is a secondary source browser rather than the default workspace. Raw activity, runtime counters, Param TX help, and the MIDI monitor are under **Diagnostics**. Saved setup JSON actions are under **Mappings → Manage saved controller setup**.
...
For a continuous LFO- or PolyTouch-driven Palette offset, create a Mod Matrix route with the desired modulation source and an unused MIDI CC as destination on the selected MIDI channel. In **Mappings → Add mapping**, either generate the signal and use the detected source or expand **Enter MIDI address manually**. Choose Colour → Palette offset, tune its response, save, and arm it. The application follows incoming CC values; it does not synthesize the LFO locally or consume raw polyphonic-aftertouch messages. Prefer a CC that is not also being sent by another controller or Hydrasynth parameter.
...
With Primary playing, arm the desired mapping or mappings, then open **Takes** and choose **Record new take**. Recording and replay state also remains visible in the workspace header and Live view. Stop recording to save a timestamped local take; choose **Replay saved take** while Primary is playing to apply it even with the Explorer disconnected. Takes remain distinct from controller setup, layouts, Waypoints and Journeys; see [PERFORMANCE_TAKES.md](PERFORMANCE_TAKES.md).
