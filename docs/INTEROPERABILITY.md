...
This is architecture guidance, not an implemented external-control API. The original post-9.2 audit remains applicable after Phase 9.3 completion. ADR-026 records the semantic boundaries; ADR-028 adds shared-world workspace ownership. The [Phase 10.0 design](WORKSPACE_DESIGN.md) and ADR-029 specify ownership and persistence responsibilities. Phase 10.1 now implements only the [two-target semantic seam](SEMANTIC_PARAMETERS.md); external integrations remain unscheduled.
