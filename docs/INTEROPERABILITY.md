...
This is architecture guidance, not an implemented external-control API. ADR-026 records the semantic boundaries; ADR-028 adds shared-world workspace ownership. The [Phase 10.0 design](WORKSPACE_DESIGN.md) and ADR-029 specify ownership and persistence responsibilities. Phase 10.1 implements the [two-target semantic seam](SEMANTIC_PARAMETERS.md); 10.2 implements [internal explicit-time evaluation](MODULATION_EVALUATION.md) under ADR-031. Real Synth is the next scheduled first external integration; no external-input runtime exists yet.
...
The current evaluator accepts only internal sources. `parameters/validateRenderConfig.ts` supplies the final boundary for current typed evaluated frames: reject non-finite data, compose domain normalisers/validators, and resolve material compatibility without modifying authored state. It is not a universal JSON validator or an external-source contract. Discrete changes, controller input lifetimes and replay of external data require their own later decisions. See [the implementation findings](MODULATION_EVALUATION.md) for program versioning, mapping issues and remaining limits.
...
5. **Phase 11.1 — Real Synth is the next scheduled integration slice:** choose the physical Real Synth and one useful mapping, passing its live inputs through the existing source/transform/mapping boundary. Keep adapters replaceable; later software adapters use the proven interaction model. Define permissions, connection lifecycle, input timing, bounded update rate, and recording/replay before treating live inputs as part of a Journey. Deterministic Journey export must use recorded or otherwise reproducible inputs, not live hardware sampled opportunistically. Audio/guitar experiments are later sources, never dependencies of fractal mathematics.
