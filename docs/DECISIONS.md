...
Represent a live absolute controller with a serialisable `ExternalControlSourceIdentity`: stable source ID, device-profile/control identity, protocol-neutral `ControlAddress`, channel and documented range. Browser input IDs, ports, permissions, callbacks, connection status and pending samples are runtime state. Pair the source with a small mapping whose target is discriminated as either a semantic parameter or a navigation intent. Do not add these relationships to `RenderConfig`; Phase 11.5 owns any persisted performance-setup collection.

Move the existing stateless scale, offset, invert, clamp and signed-curve transforms to a shared domain module used unchanged by internal modulation and external control. Keep internal explicit-time smoothing in its deterministic evaluator; live frame history is a different semantic and is not implemented. The Hydrasynth adapter decodes MIDI into absolute timestamped samples, but MIDI and device concepts stop at the adapter/profile boundary.

Coalesce dense absolute input to the latest sample per source per animation frame, cap pending sources at 32, reject older timestamps and expose received/emitted/coalesced/stale/overflow diagnostics. Palette output enters the existing temporary semantic override and final validation path. Continuous Zoom emits a signed rate intent; turn mode quantises absolute position before deriving relative navigation intent. Neither is persisted or written back to authored state.

Disconnect and input changes release and disarm effective control, clear decoder/pending/relative baselines, and retain the session relationship for explicit rebind. Reconnection never silently resumes motion. This proves one navigation target and one continuous semantic parameter without introducing events, discrete state, feedback, recording, a universal source schema, or Patch. See [EXTERNAL_CONTROL.md](EXTERNAL_CONTROL.md).
