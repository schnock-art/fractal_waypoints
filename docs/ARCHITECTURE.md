...
The device profile and selectable control catalogue are delivered. Next is completing Phase 11.3 visual semantic assignment beyond zoom, then the smallest external-source generalisation that real mappings require, then setup persistence and deterministic recorded replay. A profile owns physical identity, label/kind, protocol-neutral `ControlAddress`, and visual placement; an external adapter owns browser/device interaction; domain mappings own source → transforms → semantic target relationships. Zoom remains a relative navigation intent rather than a numeric target merely to make a future graph look uniform. Live connection state belongs to runtime; profiles, setup, takes, and future Patch layout have independent versioned ownership. Patch remains later: it presents proven wiring rather than creating an alternative architecture.
...
Hydrasynth browser lifecycle is isolated in `integrations/midi/useMidiControls.ts`; `controlInput.ts` assembles complete CC/NRPN values per input/channel before mapping. The device profile names the Explorer CC catalogue and documented Macro/Filter NRPN aliases. The UI holds one session-only assignment per current target: Zoom and Palette offset can be armed simultaneously, and replacing/releasing one does not alter the other. Zoom's default continuous mode converts documented input ranges to signed speed with a centre dead zone; frame-time accumulation emits existing navigation intents without requiring further input messages. Optional turn mode differences successive values. Palette offset maps a bounded input through an explicit finite range (optionally inverted) and calls the Perform semantic-override seam; it does not edit `RenderConfig`. Hold clears zoom speed while retaining the temporary viewport; Stop/input loss/workspace exit clear MIDI-owned viewport and semantic overrides. Neither mode increases renderer precision. Unknown packed NRPNs remain diagnostic-only. See [coverage and setup](HYDRASYNTH_CONTROLS.md); no persistence schema or renderer contract changes.
