...
Phases 11.2–11.4 now provide the device profile, selectable catalogue, visual semantic assignment and bounded external-source runtime. `connections/externalControl.ts` owns serialisable source identity, relationships, target-output discrimination and frame coalescing; `connections/signalTransforms.ts` owns stateless transforms shared with internal modulation. A profile owns physical identity, label/kind, protocol-neutral `ControlAddress`, and visual placement; an external adapter owns browser/device interaction. Zoom remains a navigation intent rather than a numeric target merely to make a future graph look uniform. Live connection state belongs to runtime; profiles, setup, takes, and future Patch layout have independent versioned ownership. See [EXTERNAL_CONTROL.md](EXTERNAL_CONTROL.md) and ADR-034. Setup persistence and deterministic recorded replay remain next. Patch presents proven wiring rather than creating an alternative architecture.
...
Hydrasynth browser lifecycle is isolated in `integrations/midi/useMidiControls.ts`; `controlInput.ts` assembles complete CC/NRPN values per input/channel before adapting them to the external-control boundary. The device profile names the Explorer CC catalogue and documented Macro/Filter NRPN aliases. The UI holds one session-only relationship per current target: Zoom and Palette offset can be armed simultaneously, and replacing/releasing one does not alter the other. Dense absolute updates coalesce to the latest value per source per animation frame with bounded diagnostics. Zoom's default continuous mode emits signed rate intents with a centre dead zone; frame-time accumulation invokes existing navigation. Optional turn mode quantises positions before deriving relative intents. Palette offset uses shared scale/offset/invert/curve/clamp transforms and the Perform semantic-override seam; it does not edit `RenderConfig`. Disconnect releases effective values but retains the serialisable session relationship, disarmed, for explicit rebind. Neither mode increases renderer precision. Unknown packed NRPNs remain diagnostic-only. See [coverage and setup](HYDRASYNTH_CONTROLS.md); no persistence schema or renderer contract changes.
