...
Phases 11.2–11.4 provide the device profile, selectable catalogue, visual semantic assignment and bounded external-source runtime. `connections/externalControl.ts` owns serialisable source identity, relationships, target-output discrimination and frame coalescing; `connections/signalTransforms.ts` owns stateless transforms shared with internal modulation. Phase 11.5 adds `connections/performanceSetup.ts`, an independently versioned setup document containing profile references and validated bindings only. Phase 11.7 adds `connections/controllerLayout.ts`: independently versioned user-authored labelled address/channel lanes, resolved only for active UI presentation and assignment convenience. Browser ports, arming/live values, world documents, takes and Patch layout remain separate. A profile owns physical identity, label/kind, protocol-neutral `ControlAddress`, and visual placement; an external adapter owns browser/device interaction. Zoom remains a navigation intent rather than a numeric target merely to make a future graph look uniform. See [EXTERNAL_CONTROL.md](EXTERNAL_CONTROL.md), [PERFORMANCE_SETUP.md](PERFORMANCE_SETUP.md), [CONTROLLER_LAYOUTS.md](CONTROLLER_LAYOUTS.md), ADR-034, ADR-035 and ADR-037. Patch presents proven wiring rather than creating an alternative architecture.
...
Phase 10.1 incrementally supplies semantic metadata; 10.2 evolves internal modulation and final domain validation; 10.3 delivers Perform; 10.4 establishes capability-aware analysis snapshots. Existing metadata, validation, trap-slot addressing, and metric-output limitations are detailed in [INTEROPERABILITY.md](INTEROPERABILITY.md), not solved by this document. The Hydrasynth Explorer sequence is the next scheduled Phase 11 work: one physical device will validate the shared input-adapter boundary without embedding synth knowledge in formulas, materials, or renderers. Patch Bay (12) remains unscheduled. Deep zoom, 3D, and specialist mathematical models are independent research tracks in [ROADMAP.md](ROADMAP.md#optional-parallel-research-tracks), not hidden workspace backends.
...
src/connections/     External source, shared transform, and controller-setup contracts
...
Hydrasynth browser lifecycle is isolated in `integrations/midi/useMidiControls.ts`; `controlInput.ts` assembles complete CC/NRPN values per input/channel before adapting them to the external-control boundary. The controller workspace is split into intent-based presentation components for Live, Mappings, Layouts, Takes, and Diagnostics; this UI boundary does not merge their ownership domains. The device profile names the Explorer CC catalogue and documented Macro/Filter NRPN aliases. The UI holds one relationship per current target: Zoom, Palette offset, Julia Real and Julia Imaginary can be armed independently. Julia targets write session-only linked-view overrides and are availability-gated until a Julia view is linked. Relationships save automatically in the controller setup, load unarmed, and can be exported/imported; transient ports and live state never persist. A separate `PerformanceControlTake` snapshots armed relationships with ordered relative-time absolute samples and replays them through the same route without an input port. Dense absolute updates coalesce to the latest value per source per animation frame with bounded diagnostics. Zoom's default continuous mode emits signed rate intents with a centre dead zone; frame-time accumulation invokes existing navigation. Optional turn mode establishes its first baseline immediately before frame coalescing, quantises positions, then derives relative intents. Palette and Julia values use shared scale/offset/invert/curve/clamp transforms and the Perform semantic-override seam; they do not edit authored `RenderConfig`. Disconnect releases effective values but retains the relationship, disarmed, for explicit rebind. Neither mode increases renderer precision. Unknown packed NRPNs remain diagnostic-only. See [coverage, setup and takes](HYDRASYNTH_CONTROLS.md), [controller workspace UX](CONTROLLER_WORKSPACE.md); the render/Waypoint schema and renderer contracts are unchanged.
