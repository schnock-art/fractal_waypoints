...
Real Synth integration is underway in Phase 11. The Hydrasynth Explorer has a focused control editor, named CC catalogue, supported Macro/Filter NRPN inputs and simultaneous Zoom and Palette offset relationships. Continuous zoom treats knob position as speed, with centre/Hold stopping motion without resetting the view; it removes the knob-range constraint, not numerical deep-zoom limits. Phase 11.4 provides the bounded shared external-source boundary: stable source identity, shared transforms, frame-coalesced absolute updates, diagnostics, and distinct semantic values versus navigation intents. Phase 11.5 adds an independently versioned controller setup with local save, absent-device inspection, equivalent-input rebind and JSON import/export; it never adds hardware requirements to a Waypoint. Phase 11.6 adds a separate, locally replayable take that snapshots armed mappings and ordered source samples, so replay does not require the original hardware. Phase 11.7 adds reusable user-owned controller layouts: named address/channel lanes with optional patch notes that improve readability without changing source provenance, mapping identity or take replay. Later guitar/audio analysis can supply independent signals without making fractal code instrument-aware. A future node graph exposes the proven model, rather than defining it; feedback requires deliberate scheduling and replay semantics. No MIDI, audio, synth, OSC, node editor, or feedback implementation belongs in Phase 10; see ADR-026. The Schnock Generative Instrument is a long-term conceptual umbrella, not a rename, monorepo, or framework commitment.
