...
Real Synth integration is underway in Phase 11. The Hydrasynth Explorer now has a focused control editor, named CC catalogue, supported Macro/Filter NRPN inputs and one session-only zoom assignment. Continuous zoom treats knob position as speed, with centre/Hold stopping motion without resetting the view; it removes the knob-range constraint, not numerical deep-zoom limits. The next slice completes visual assignment beyond zoom using an existing semantic parameter. Shared external-source generalisation, saved setups and deterministic take replay remain later steps; real hardware should inform a model that a later software adapter can also use. Later guitar/audio analysis can supply independent signals without making fractal code instrument-aware. A future node graph exposes the proven model, rather than defining it; feedback requires deliberate scheduling and replay semantics. No MIDI, audio, synth, OSC, node editor, or feedback implementation belongs in Phase 10; see ADR-026. The Schnock Generative Instrument is a long-term conceptual umbrella, not a rename, monorepo, or framework commitment.
