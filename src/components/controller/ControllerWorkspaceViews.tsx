...
import { midiAssignmentTargets, midiTargetLabel, midiTargetRange, type MidiAssignmentTarget, type MidiControlsApi } from '../../integrations/midi/useMidiControls';
...
export function LiveControllerView({ midi, layouts, active, running, onNavigate }: LiveProps) {
  const assignments = midiAssignmentTargets.filter((target) => midi.assignments[target]).length;
  return <section className="controller-view controller-live" aria-labelledby="controller-live-title">
    ...
    <div className="controller-card-grid">
      {midiAssignmentTargets.map((target) => <ControllerMappingCard key={target} assignment={midi.assignments[target]} target={target} armed={midi.isArmed(target)} layout={layouts.activeLayout}
        canArm={!!midi.selectedId && midi.isTargetAvailable(target)} unavailableReason={!midi.isTargetAvailable(target) ? 'Use Julia as Primary or link a Julia view in Explore before arming this mapping.' : undefined}
        onToggleArm={() => midi.toggleArm(target)} onEdit={() => onNavigate('mappings')} onRelease={() => midi.releaseTarget(target)} />)}
    </div>
    ...
    <section className="controller-take-summary" aria-label="Performance take">
      ...
      <div className="controller-actions">
        {midi.isRecordingTake ? <button type="button" onClick={() => midi.stopRecordingTake()}>Stop recording</button>
          : <button type="button" disabled={!active || !running || !midi.selectedId || !Object.values(midi.armedTargets).some(Boolean)} onClick={midi.startRecordingTake}>● Record take</button>}
...
export function MappingControllerView({ midi, layouts }: SharedProps) {
  ...
  const rangeValid = target === 'zoom' || Number.isFinite(minimum) && Number.isFinite(maximum) && minimum !== maximum && Number.isFinite(curve) && curve > 0 && curve <= 8;
  const edit = (nextTarget: MidiAssignmentTarget) => {
    ...
    if (assignment) {
      ...
      const [defaultMinimum, defaultMaximum] = midiTargetRange(nextTarget);
      setMinimum(assignment.minimum ?? defaultMinimum); setMaximum(assignment.maximum ?? defaultMaximum); setCurve(assignment.curve ?? 1); setInverted(assignment.inverted ?? false);
    } else { const [defaultMinimum, defaultMaximum] = midiTargetRange(nextTarget); setMinimum(defaultMinimum); setMaximum(defaultMaximum); setSource(null); }
  };
  const useLast = () => {
  ...
  return <section className="controller-view" aria-labelledby="controller-mappings-title">
    ...
    <div className="controller-card-grid">
      {midiAssignmentTargets.map((target) => <ControllerMappingCard key={target} assignment={midi.assignments[target]} target={target} armed={midi.isArmed(target)} layout={layouts.activeLayout}
        canArm={!!midi.selectedId && midi.isTargetAvailable(target)} unavailableReason={!midi.isTargetAvailable(target) ? 'Use Julia as Primary or link a Julia view in Explore before arming this mapping.' : undefined}
        onToggleArm={() => midi.toggleArm(target)} onEdit={() => edit(target)} onRelease={() => midi.releaseTarget(target)} onRemove={() => midi.removeAssignment(target)} />)}
    </div>
    {editing ? <section className="mapping-editor" aria-label="Add mapping">
      ...
      <section><span className="control-panel__label">1 · Source</span><h4>Move or send a control</h4>
        ...
        {layouts.activeLayout?.lanes.length ? <details><summary>Use a lane from {layouts.activeLayout.name}</summary><div className="controller-lane-grid" aria-label="Available controller layout lanes">{layouts.activeLayout.lanes.map((lane) => <article key={lane.id}><div><strong>{lane.label}</strong><small>{addressLabel(lane.address)} · Channel {lane.channel + 1}</small></div><button type="button" onClick={() => setSource({ address: lane.address, channel: lane.channel, learned: !findHydrasynthExplorerControl(lane.address), label: lane.label })}>Use lane</button></article>)}</div></details> : null}
      ...
      <section><span className="control-panel__label">2 · Target</span><h4>Choose what moves</h4><label>Controller target<select aria-label="Controller target" value={target} onChange={(event) => { const next = event.target.value as MidiAssignmentTarget; setTarget(next); const [nextMinimum, nextMaximum] = midiTargetRange(next); setMinimum(nextMinimum); setMaximum(nextMaximum); }}><optgroup label="Navigation"><option value="zoom">Zoom</option></optgroup><optgroup label="Colour"><option value="palette.offset">Palette offset</option></optgroup><optgroup label="Julia"><option value="formula.julia.cReal">Julia Real</option><option value="formula.julia.cImag">Julia Imaginary</option></optgroup></select></label>{!midi.isTargetAvailable(target) ? <small className="perform-warning">{midiTargetLabel(target)} can be configured now, but requires Julia as Primary or a linked Julia view before it can be armed.</small> : null}</section>
      <section><span className="control-panel__label">3 · Response</span><h4>Shape the feel</h4>
        {target === 'zoom' ? <fieldset className="response-options"><legend>Zoom behaviour</legend><label><input type="radio" name="zoom-mode" checked={midi.zoomMode === 'continuous'} onChange={() => midi.changeZoomMode('continuous')} /> Continuous speed</label><label><input type="radio" name="zoom-mode" checked={midi.zoomMode === 'turn'} onChange={() => midi.changeZoomMode('turn')} /> Zoom while turning</label><small>{midi.zoomMode === 'continuous' ? 'Centre holds the view. Further from centre moves faster and continues at the knob limit.' : 'The first value establishes a baseline; subsequent turns move relative to it.'}</small></fieldset>
          : <div className="response-options"><div className="response-range"><label>Output minimum<input aria-label={`${midiTargetLabel(target)} minimum`} type="number" step="0.01" value={minimum} onChange={(event) => setMinimum(event.target.valueAsNumber)} /></label><label>Output maximum<input aria-label={`${midiTargetLabel(target)} maximum`} type="number" step="0.01" value={maximum} onChange={(event) => setMaximum(event.target.valueAsNumber)} /></label></div><label>Response curve <input aria-label={`${midiTargetLabel(target)} response curve`} type="range" min="0.1" max="8" step="0.1" value={curve} onChange={(event) => setCurve(event.target.valueAsNumber)} /><output>{curve.toFixed(1)}</output></label><label className="inline-check"><input aria-label={`Invert ${midiTargetLabel(target)}`} type="checkbox" checked={inverted} onChange={(event) => setInverted(event.target.checked)} /> Invert response</label></div>}
      </section>
    ...
    <details className="controller-manage"><summary>Manage saved controller setup</summary><small role="status">{midi.setupStatus}</small><div className="controller-actions"><button type="button" disabled={!midiAssignmentTargets.some((target) => midi.assignments[target])} onClick={midi.exportSetup}>Export setup JSON</button><button type="button" onClick={() => setupFileRef.current?.click()}>Import setup JSON</button><button type="button" className="is-danger" disabled={!midiAssignmentTargets.some((target) => midi.assignments[target])} onClick={midi.clearSetup}>Clear saved setup</button></div><input ref={setupFileRef} className="midi-setup__file" aria-label="Import controller setup JSON" type="file" accept="application/json,.json" onChange={importSetup} /></details>
  </section>;
}
