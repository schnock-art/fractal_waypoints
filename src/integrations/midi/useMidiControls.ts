...
export function useMidiControls(active: boolean, running: boolean, onZoomDelta: (delta: number) => void, onPaletteOffset: (value: number) => void, onRelease: (target?: MidiAssignmentTarget) => void) {
  ...
  const changeZoomMode = (mode: 'continuous' | 'turn') => { holdZoom(); runtime.current.mode = mode; setZoomMode(mode); };
  ...
  const release = () => { holdZoom(); runtime.current.armed = unarmed(); setArmedTargets(unarmed()); callbacks.current.onRelease(); };
  const selectInput = (id: string) => {
    release(); runtime.current.selectedId = id; runtime.current.assignments = {};
    setSelectedId(id); setAssignments({}); setMessages([]); setLastInput(null); setTraffic('No MIDI received yet.');
    setStatus(id ? 'Turn a control on the synth to identify it.' : 'Choose an input to continue.');
  };
  useEffect(() => {
    mounted.current = true;
    return () => { mounted.current = false; for (const input of access.current?.inputs.values() ?? []) input.onmidimessage = null; if (access.current) access.current.onstatechange = null; callbacks.current.onRelease(); };
  }, []);
  ...
  useEffect(() => {
    ...
    input.onmidimessage = (event) => {
      ...
      for (const target of targets) {
        const assignment = current.assignments[target];
        if (!current.armed[target] || !assignment || inputKey(assignment) !== inputKey(value)) continue;
        if (target === 'palette.offset') {
          const mapped = controlRangeValue(value.value, controlMaximum(value.address), assignment.minimum ?? -1, assignment.maximum ?? 1, assignment.inverted);
        ...
        if (current.mode === 'continuous') {
          ...
          current.rate = rate; setZoomRate(rate);
        } else {
          const position = controlZoomPosition(value.value, controlMaximum(value.address)); const previous = current.previous; current.previous = position;
  ...
  const assign = (value: Assignment) => {
    ...
    runtime.current.assignments = { ...runtime.current.assignments, [value.target]: value }; setAssignments({ ...runtime.current.assignments });
  ...
  return { inputs, selectedId, connected: access.current !== null, connecting, connect, selectInput, assignments, assign, armedTargets, isArmed: (target: MidiAssignmentTarget) => armedTargets[target], toggleArm, release, releaseTarget, messages, lastInput, traffic, status, zoomMode, changeZoomMode, zoomRate, holdZoom };
}
